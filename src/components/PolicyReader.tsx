// src/components/PolicyReader.tsx
// Reads ?h=... from the URL, decodes the policy, and renders it.
import { useEffect, useMemo, useState } from 'react';
import { generatePolicy, decodeHashToPolicy } from '@/lib/generator';
import { PolicyView } from '@/components/PolicyView';
import { I18nProvider, useI18n } from '@/lib/I18nProvider';
import { localizePath } from '@/lib/i18n';
import { buildGeneratorOverrides } from '@/lib/buildOverrides';
function PolicyReaderInner() {
  const { t, locale } = useI18n();
  const [error, setError] = useState<string | null>(null);
  const [hash, setHash] = useState<string | null>(null);

  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const h = params.get('h');
    if (!h) {
      setError(t('policyReader.errorNoHash'));
      return;
    }
    const decoded = decodeHashToPolicy(h);
    if (!decoded) {
      setError(t('policyReader.errorInvalid'));
      return;
    }
    setHash(h);
  }, [t]);

  const policy = useMemo(() => {
    if (typeof window === 'undefined') return null;
    const h = new URLSearchParams(window.location.search).get('h');
    if (!h) return null;
    const decoded = decodeHashToPolicy(h);
    if (!decoded) return null;
    return generatePolicy(decoded, buildGeneratorOverrides(t, locale));
  }, [hash]);

  const formattedDate = useMemo(() => {
    if (!policy) return '';
    return new Date(policy.generatedAt).toLocaleDateString(locale, {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    });
  }, [policy, locale]);

  if (error) {
    return (
      <div className="rounded-card border border-border bg-bg-elevated p-8 text-center">
        <p className="text-fg-muted text-sm mb-4">{error}</p>
        <a
          href={localizePath('/generate', locale)}
          className="inline-flex h-9 items-center rounded-button bg-fg text-bg px-4 text-sm font-medium hover:opacity-90"
        >
          {t('policyReader.generateNew')}
        </a>
      </div>
    );
  }

  if (!policy) {
    return (
      <div className="rounded-card border border-border bg-bg-elevated p-8 text-center">
        <div className="inline-block h-5 w-5 border-2 border-fg-faint border-t-fg rounded-full animate-spin" />
        <p className="text-fg-muted text-sm mt-3">{t('policyReader.loading')}</p>
      </div>
    );
  }

  return (
    <div className="space-y-6">
      <div className="rounded-card border border-border bg-bg-elevated p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold tracking-tight">
            {t('policyReader.title', { businessName: policy.businessName })}
          </h1>
          <p className="text-xs text-fg-muted mt-0.5">
            {t('policyReader.lastUpdated', { date: formattedDate })}
          </p>
        </div>
        <a
          href={localizePath('/generate', locale)}
          className="inline-flex h-8 items-center rounded-button border border-border bg-bg px-3 text-xs font-medium text-fg hover:bg-bg-card"
        >
          {t('policyReader.generateOwn')}
        </a>
      </div>
      <PolicyView policy={policy} />
    </div>
  );
}

export function PolicyReader() {
  return (
    <I18nProvider>
      <PolicyReaderInner />
    </I18nProvider>
  );
}
