// src/components/TermsReader.tsx
// Reads ?h=... from the URL, decodes the terms input, generates the document, and renders it.

import { useEffect, useMemo, useState } from 'react';
import { decodeHashToTerms, generateTerms } from '@/lib/terms-generator';
import { DocumentView } from '@/components/DocumentView';
import { I18nProvider, useI18n } from '@/lib/I18nProvider';
import { localizePath } from '@/lib/i18n';
import { buildTermsOverrides } from '@/lib/buildOverrides';

function TermsReaderInner() {
  ...
  useEffect(() => {
    ...
    if (!h) {
      setError(t('termsReader.errorNoHash'));
      return;
    }
    const decoded = decodeHashToTerms(h);
    if (!decoded) {
      setError(t('termsReader.errorInvalid'));
      return;
    }
    setHash(h);
  }, [t]);
  ...
  const doc = useMemo(() => {
    ...
    return generateTerms(decoded, buildTermsOverrides(t, locale));
  }, [hash]);
  ...
  if (!doc) {
    return (
      <div className="rounded-card border border-border bg-bg-elevated p-8 text-center">
        ...
        <p className="text-fg-muted text-sm mt-3">{t('termsReader.loading')}</p>
      </div>
    );
  }
      ...
      <div className="rounded-card border border-border bg-bg-elevated p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold tracking-tight">
            {t('termsReader.title', { businessName: doc.businessName })}
          </h1>
          <p className="text-xs text-fg-muted mt-0.5">
            {t('termsReader.lastUpdated', { date: formattedDate })}
          </p>
        </div>
        <a
          href={localizePath('/generate/terms', locale)}
        ...
        >
          {t('termsReader.generateOwn')}
        </a>
      </div>
      <DocumentView
        title={t('documentType.terms')}
